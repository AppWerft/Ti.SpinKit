...
static Handle<Value> Spin_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Spin.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::SpinBindings::lookupGeneratedInit(
...
static void Spin_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Spin_getBinding)->GetFunction());
}

static void Spin_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::SpinBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry SpinBinding = {
	"de.appwerft.spinkit",
	Spin_init,
	Spin_dispose
};
...
Java_de_appwerft_spinkit_SpinBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.spinkit", &SpinBinding);
	titanium::KrollBindings::addExternalLookup(&(::SpinBindings::lookupGeneratedInit));
}
